...
function renderStats() {
  ...
  document.getElementById("stat-top-value").textContent =
    `${values[maxIdx]} 票`;
  document.getElementById("stat-low-label").textContent = labels[minIdx];
  document.getElementById("stat-low-value").textContent =
    `${values[minIdx]} 票`;

  const total2 = total || 1;
  document.getElementById("breakdown").innerHTML = labels
    .map((label, i) => {
